...
### 6.1 Deferred prose/doc cleanup (booked for Plans 3–4)

Plan 2 removes the context **mechanism** (code, schemas, source registries, templates,
hook, frontmatter field, generation) and was verified clean for those tokens across
`framework/modules` + `framework/cli/src`. The following **descriptive prose/docs** still
reference the old system and are deliberately deferred:

- **Plan 3 (knowledge skills):** Rewrite each project-facing agent's `## Required Reading`
  / `### Context Files` section — they still instruct "review `.claude/context/*.md`" (files
  that no longer exist and that `init` no longer creates). Replace with guidance to invoke
  the relevant `knowing-*` skill (§5). This is deferred because it requires the `knowing-*`
  skills to exist first.
- **Plan 4 (re-baseline + doc rewrite):** User-facing docs still describe the removed context
  system (and the pruned writer/reporting/iOS modules): `CLAUDE.md`, `README.md`, `docs/*`,
  `CHANGELOG.md`, the `integrations/openclaw/*` manifest, and `routes.yml` (removed in Plan 4
  per §8). `.serena/memories/*` are local dev notes, not framework deliverables.

### 6.2 The self-deployed `.claude/` tree (resolved in Plan 2)

The repo bootstrap copied a committed `.claude/` self-deployment (the framework dogfooding
itself). It was a stale mirror of the old framework. **Decision:** stop tracking the generated
`.claude/` deployment (agents, commands, context, hooks, registries, skills, settings.json,
statusline.sh, .current_agent) — it is regenerated by `init`/`sync` from
`framework/modules` + `framework/cli` and is not source of truth. `settings.local.json`
remains user-owned. Done in Plan 2.
